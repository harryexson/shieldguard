...
function SmsScannerBody() {
  const [text, setText] = React.useState('');
  const [result, setResult] = React.useState<SmsResult | null>(null);
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState('');

  const analyze = async () => {
    if (!text.trim()) {
      setError('Paste an SMS message to analyze.');
      return;
    }
    setError('');
    setLoading(true);
  ...
  return (
    <View style={styles.section}>
      <Text style={styles.sectionTitle}>SMS Phishing Analysis</Text>
      <TextInput
        style={styles.input}
      ...
      <TouchableOpacity style={styles.button} onPress={analyze} disabled={loading}>
        {loading ? <ActivityIndicator color="#04121E" /> : <Text style={styles.buttonText}>Analyze message</Text>}
      </TouchableOpacity>
...
export function SMSSecurityScreen() {
  const [smsFiltering, setSmsFiltering] = React.useState(true);

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <Text style={styles.icon}>💬</Text>
        <Text style={styles.title}>SMS & Text Security</Text>
        <Text style={styles.subtitle}>Heuristic detection of malicious SMS, phishing and scams</Text>
      </View>

      <View style={styles.statusCard}>
        <Text style={styles.statusLabel}>Engine</Text>
        <Text style={styles.statusValue}>Backend-connected</Text>
        <Text style={styles.statusDetail}>Analyzes message content for known threat patterns</Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.featureItem}>
          <View style={styles.featureInfo}>
            <Text style={styles.featureName}>Live SMS scanning</Text>
            <Text style={styles.featureDesc}>Routes incoming message text through the detection engine</Text>
          </View>
          <Switch value={smsFiltering} onValueChange={setSmsFiltering} trackColor={{ false: COLORS.border, true: COLORS.safe + '80' }} thumbColor={smsFiltering ? COLORS.safe : COLORS.textSecondary} />
        </View>
      </View>

      <FeatureGate feature="sms_scan">
        <SmsScannerBody />
      </FeatureGate>
    </ScrollView>
  );
